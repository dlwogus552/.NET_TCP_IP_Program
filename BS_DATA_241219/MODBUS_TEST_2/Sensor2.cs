...
namespace MODBUS_TEST_2
{
    public partial class Sensor2 : Form
    {
        ...
        public float[] preValues = new float[8];

        TextBox[] txPreArray = new TextBox[8];
        DBHelper helper = new DBHelper();

        public Sensor2()
        {
            InitializeComponent();
            socketConnecter = new SocketConnecter("172.30.1.57", 8899, "sen 2");
            // 연결 성공 시 타이머 시작
            socketConnecter.OnConnected += () =>
            {
                Log("sen 2 소켓 연결 성공 이벤트 발생. 타이머 시작.");
                ...
                if (InvokeRequired)
                {
                    Log($"sen 2 Invoke 호출 시작 : {timer2.Enabled}");

                    Invoke(new Action(() =>
                    {
                        if (!timer2.Enabled)
                        {
                            Log("sen 2 타이머 시작(Invoke)");
                            timer2.Start();
                        }
                    }));
                }
            ...
            txPreArray[2] = Tx_Pre_3;
            txPreArray[3] = Tx_Pre_4;
            txPreArray[4] = Tx_Pre_5;
            txPreArray[5] = Tx_Pre_6;
            txPreArray[6] = Tx_Pre_7;
            txPreArray[7] = Tx_Pre_8;
        ...
        void DataReceived(IAsyncResult ar)
        {
            try
            {
                ...
                Log($"Sen 2 데이터 받음 - 수신된 데이터: {received}, 버퍼: {BitConverter.ToString(receivedData)}");
                if (slaveCheck <= 8)
                {
            ...
            catch (ObjectDisposedException)
            {
                ...
                Log("Sen 2 소켓이 폐기되었습니다.");
            }
            catch (ArgumentOutOfRangeException ex)
            {
                Log($"Sen 2 DataReceived 오류: {ex}");
                socketConnecter.obj.WorkingSocket.BeginReceive(socketConnecter.obj.Buffer, 0, socketConnecter.obj.BufferSize, 0, DataReceived, socketConnecter.obj);
            }
            catch (Exception ex)
            {
                Log($"Sen 2 DataReceived 오류: {ex}");
                helper.inesrtError(ex.GetType().ToString(), "DataReceived 에러 - " + ex.Message, "sen 2");
                lock (socketConnecter.lockObject)
                {
        ...
        public float HextoFloat(string num, int index)
        {
        ...
        private void UpdatePreTextBox()
        {
            for (int i = 0; i < preValues.Length; i++)
            {
                if (InvokeRequired)
                {
                    Invoke(new Action(() => txPreArray[i].Text = preValues[i].ToString()));
                }
                else
                {
                    txPreArray[i].Text = preValues[i].ToString();
                }
            }
        }
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            List<byte[]> sendDataList = new List<byte[]>
             {
               ...
               new byte[] { 0x03, 0x04, 0x00, 0x00, 0x00, 0x01 },
               new byte[] { 0x04, 0x04, 0x00, 0x00, 0x00, 0x01 },
               new byte[] { 0x05, 0x04, 0x00, 0x00, 0x00, 0x01 },
               new byte[] { 0x06, 0x04, 0x00, 0x00, 0x00, 0x01 },
               new byte[] { 0x07, 0x04, 0x00, 0x00, 0x00, 0x01 },
               new byte[] { 0x08, 0x04, 0x00, 0x00, 0x00, 0x01 },
             };
            ...
            foreach (var sendData in sendDataList)
            {
                ...
                catch (Exception ex)
                {
                    // 예외 처리
                    Log($"데이터 전송 오류: {ex}");
                    helper.inesrtError(ex.GetType().ToString(), "Data Send 에러 - " + ex.Message, "sen 2");
            ...
            ParseValues("02", preValues, "압력", 14);
        }
        private void ParseValues(string suffix, float[] values, string unit, int receiveDataLength)
        {
            if (resultMsg.Length == 0) return;

            for (int i = 0; i < 8; i++)
            {
                int valueStart = resultMsg.IndexOf($"0{i+1}04{suffix}");
                if (valueStart != -1 && resultMsg.Length - valueStart >= receiveDataLength)
                {
                    values[i] = HextoFloat(resultMsg.Substring(valueStart, receiveDataLength).Substring(6, 4), i);
                    Log($"처리한 {unit} 데이터 {i + 1} : {values[i]}");
                    resultMsg = resultMsg.Substring(valueStart + receiveDataLength);
                }
            }
            UpdatePreTextBox();
        }
