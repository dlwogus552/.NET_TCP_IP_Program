using MySqlX.Protocol;
using Newtonsoft.Json.Linq;
using Org.BouncyCastle.Asn1.Ocsp;
using Org.BouncyCastle.Tls;
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Data;
using System.Diagnostics;
using System.Drawing;
using System.Globalization;
using System.Linq;
using System.Net;
using System.Net.Sockets;
using System.Reflection;
using System.Text;
using System.Threading;
using System.Threading.Tasks;
using System.Windows.Forms;
using YJ_SENSOR_DATA;

namespace MODBUS_TEST_2
{
    public partial class Sensor4 : Form
    {
        SocketConnecter socketConnecter = null;
        bool sendCheck = false;

        public float[] kwhValues;
        public float[] wValues = new float[8] { 0, 0, 0, 0, 0, 0, 0, 0 };
        List<Label> txPerArray = new List<Label>();
        private List<int> pendingRequests = new List<int>();
        private DBHelper dbHelper = new DBHelper();
        public int delayTime = 60000;
        string resultMsg = "";
        // 데이터 전송을 위한 바이트 배열 정의
        private List<byte[]> sendDataList = new List<byte[]> {
                        new byte[] { 0x00, 0x04, 0x00, 0x0C, 0x00, 0x01 },//전력 W
                        new byte[] { 0x00, 0x04, 0x00, 0x1A, 0x00, 0x02 }//전체 유효 전력 wh
                    };

        public Sensor4()
        {
            InitializeComponent();
            socketConnecter = new SocketConnecter("172.30.1.56", 8899, "전력량계");

            socketConnecter.OnConnected += () =>
            {
                Log("sen 4 소켓 연결 성공 이벤트 발생. 타이머 시작.");
                sendCheck = true;
                _ = SendProtocol();
            };
            socketConnecter.OnDataReceived += ProcessData;
            // 연결 해제시 타이머 종료
            socketConnecter.DisConnected += () =>
            {
                sendCheck = false;
            };
            LoadForm();
            kwhValues = dbHelper.selectElect();
            _ = socketConnecter.ConnectServerAsync();
        }
        private void LoadForm()
        {
            for (int i = 0; i < 8; i++)
            {
                this.Controls.Add(createPanel(i));
            }

        }
        // panel 만들기
        private Panel createPanel(int i)
        {
            int x = i<4 ? i * (210 + 96) : (i-4) * (210 + 96);
            int y = i<4 ? 0:140;
            Panel panel = new Panel();
            panel.Size = new Size(210, 130);
            panel.BackColor = Color.White;
            panel.Location = new Point(x, y);

            Label textBox = new Label();
            textBox.BorderStyle = BorderStyle.None;
            textBox.Location = new Point(0, 0);
            textBox.Size = new Size(210, 31);
            textBox.Font = new Font("굴림", 20F, FontStyle.Bold);
            textBox.BackColor = Color.FromArgb(0xf7, 0xf7, 0xf7);
            textBox.Text = $"전력량 {i + 1}";
            textBox.TextAlign = ContentAlignment.MiddleCenter;

            PictureBox kWhIcon = new PictureBox();
            kWhIcon.Size = new Size(90, 90);
            kWhIcon.Padding = new Padding(10);
            kWhIcon.BackColor = Color.LightGray;
            kWhIcon.Location = new Point(4, 36);
            kWhIcon.SizeMode = PictureBoxSizeMode.StretchImage;
            kWhIcon.Image = YJ_SENSOR_DATA.Properties.Resources.power;

            Label kWhLabel = new Label();
            kWhLabel.Font = new Font("굴림", 20F, FontStyle.Bold);
            kWhLabel.AutoSize = true;
            kWhLabel.Text = "kWh";
            kWhLabel.Location = new Point(96, 51);

            Label kWhValue = new Label();
            kWhValue.Font = new Font("굴림", 20F, FontStyle.Bold);
            kWhValue.AutoSize = true;
            kWhValue.Location = new Point(96, 81);
            kWhValue.Text = "0";

            panel.Controls.Add(textBox);
            panel.Controls.Add(kWhIcon);
            panel.Controls.Add(kWhLabel);
            panel.Controls.Add(kWhValue);
            txPerArray.Add(kWhValue);

            panel.Paint += (sender, e) =>
            {
                int borderSize = 1; // 테두리 두께
                Color borderColor = Color.FromArgb(0xda, 0xda, 0xda); // 테두리 색상

                using (Pen pen = new Pen(borderColor, borderSize))
                {
                    e.Graphics.DrawRectangle(pen, new Rectangle(0, 0, panel.Width - 1, panel.Height - 1));
                }
            };
            return panel;
        }
        public string ByteToHex(byte[] hex)
        {
            StringBuilder result = new StringBuilder(hex.Length * 2);
            foreach (byte b in hex)
            {
                result.Append(b.ToString("x2").ToUpper());
            }
            return result.ToString();
        }

        public string Convert_Per(string msg,int slaveCheck,string protocolName)
        {
            Log($"sen 4 Addr : {slaveCheck} {protocolName} 수신: {msg}");

            return msg;
        }

        public float HextoFloat(string num,string name,int index)
        {
            int intRep = int.Parse(num, NumberStyles.AllowHexSpecifier); // 16진수 문자열을 정수로 파싱
            float f = intRep;// 실제 값으로 변환하기 위해 10으로 나누기
            Log($"변환 데이터 {index+1}번 {name} : {f}");
            if (f > 10000 && name == "w")
            {
                f= wValues[index];
            }
            if ((f > 10000000 || f<0) && name == "kw")
            {
                f = wValues[index];
            }
            return f;
        }
        void ProcessData(byte[] buffer)
        {
            int receiveId = buffer[0];
            string protocolName = (int)buffer[2] == 2?"전력(W)" : "전체 유효 전력 wh";
            resultMsg += Convert_Per(ByteToHex(buffer), receiveId, protocolName);
        }
        // 전력량계 데이터 밀림현상으로 문자열로 종합 후 split 하는 방식으로 진행(응답이 밀려서 다음 응답에 같이 전송되는 현상)
        // 추후 AP 설치 등 네트워크 환경 변화 후 테스트 필요
        // 네트워크 설치 이후에도 동일한 상황 발생 시 USR교체 또는 제조사 문의 필요
        /*        public string Convert_Per(string msg, int slaveCheck, string protocolName, int dataLength)
                {
                    Log($"sen 4 Addr : {slaveCheck} {protocolName} 수신: {msg}");
                    int sliceAmount = protocolName == "전체 유효 전력 wh" ? 8 : 4;
                    if (string.IsNullOrEmpty(msg) || msg.Length < 10 + dataLength * 2)
                    {
                        throw new ArgumentOutOfRangeException(nameof(msg), "sen 4 입력 문자열이 너무 짧습니다.");
                    }
                    if (msg.Length % (10 + dataLength * 2) != 0)
                    {
                        throw new ArgumentOutOfRangeException(nameof(msg), "sen 4 입력 문자열이 올바르지 않습니다.");
                    }

                    for (var i = 0; i < msg.Length; i += 10 + dataLength * 2)
                    {
                        var trimmedMsg = msg.Substring(i, 10 + dataLength * 2);
                        if (trimmedMsg.StartsWith($"0{slaveCheck}"))
                        {
                            string con = trimmedMsg.Substring(6, sliceAmount);
                            Log($"sen 4 Addr : {slaveCheck} {protocolName} 변환: {con}");
                            return con;
                        }
                    }
                    throw new ArgumentOutOfRangeException(nameof(msg), "해당 주소로 시작하는 유효한 메시지를 찾을 수 없습니다.");
                }
                public float HextoFloat(string num)
                {
                    int intRep = int.Parse(num, NumberStyles.AllowHexSpecifier); // 16진수 문자열을 정수로 파싱
                    float f = intRep;// 실제 값으로 변환하기 위해 10으로 나누기
                    return f;
                }*/


        /*private void ProcessData(byte[] buffer)
        {
            try
            {
                int slaveId = buffer[0];
                int dataLength = buffer[2];
                int requestId = (slaveId << 8) | dataLength;
                string protocolName = "";
                float value = 0;
                if (pendingRequests.Contains(requestId))
                {
                    switch (dataLength)
                    {
                        case 2:
                            protocolName = "전력(W)";
                            value = HextoFloat(Convert_Per(ByteToHex(buffer), slaveId, protocolName, dataLength));
                            if (value < 10000)
                            {
                                wValues[slaveId - 1] = value;
                            }
                            break;
                        case 4:
                            protocolName = "전체 유효 전력 wh";
                            value = HextoFloat(Convert_Per(ByteToHex(buffer), slaveId, protocolName, dataLength));
                            if (value < 4200000000)
                            {
                                kwhValues[slaveId - 1] = value;
                            }
                            break;
                    }
                    Log($"sen 4 Addr : {slaveId}번 {protocolName} 변환 값 :{value}");

                    pendingRequests.Remove(requestId);
                }
            
            }
            catch (ArgumentOutOfRangeException ex)
            {
                Log($"sen 3 데이터 수신 오류 : {ex}");
            }
            catch (IndexOutOfRangeException ex)
            {
                Log($"sen 3 데이터 수신 오류 : {ex}");
            }
        }*/

        private void UpdatePerTextBox()
        {
            if (InvokeRequired)
            {
                for (var i = 0; i < 8; i++)
                {
                    Invoke(new Action(() => txPerArray[i].Text = kwhValues[i].ToString()));
                }
            }
            else
            {
                for (var i = 0; i < 8; i++)
                {
                    txPerArray[i].Text = kwhValues[i].ToString();
                }
            }
        }

        private static ushort CalculateCRC(byte[] data, int length)
        {
            ushort crc = 0xFFFF;
            for (int i = 0; i < length; i++)
            {
                crc ^= (ushort)(data[i] & 0xFF);

                for (int j = 0; j < 8; j++)
                {
                    if ((crc & 0x0001) != 0)
                    {
                        crc >>= 1;
                        crc ^= 0xA001;
                    }
                    else
                    {
                        crc >>= 1;
                    }
                }
            }
            return crc;
        }

        private async Task SendProtocol()
        {
            try
            {

                while (sendCheck)
                {
                    resultMsg = "";
                    
                    for (var j = 0; j < sendDataList.Count; j++)
                    {
                        for (var i = 1; i <= 8; i++)
                        {
                            sendDataList[j][0] = (byte)i;
                            // CRC 계산
                            ushort crc = CalculateCRC(sendDataList[j], sendDataList[j].Length);
                            byte crcLow = (byte)(crc & 0xFF);
                            byte crcHigh = (byte)((crc >> 8) & 0xFF);

                            // CRC를 추가한 전송용 데이터 배열 생성
                            byte[] sendDataWithCRC = new byte[sendDataList[j].Length + 2];
                            Array.Copy(sendDataList[j], 0, sendDataWithCRC, 0, sendDataList[j].Length);
                            sendDataWithCRC[sendDataList[j].Length] = crcLow;
                            sendDataWithCRC[sendDataList[j].Length + 1] = crcHigh;

                            // 변환된 데이터 로그 출력
                            string sendDataString = BitConverter.ToString(sendDataWithCRC);
                            int requestId = (i << 8) | (j + 1) * 2; // 요청 id 생성 slave + 데이터 길이

                            pendingRequests.Add(requestId);
                            // 데이터 전송
                            await socketConnecter.SendAsync(sendDataWithCRC);
                            Log($"sen 4 전송 데이터 {i}: {sendDataString}");
                            await Task.Delay(300);
                        }
                    }
                    splitResult();
                    await Task.Delay(delayTime);
                }
            }
            catch (Exception ex)
            {
                Log($"데이터 전송 오류: {ex}");
                dbHelper.inesrtError(ex.GetType().ToString(), "Data Send 에러 - " + ex.Message, "인버터");
                await socketConnecter.DisconnectAsync();
            }
        }
        private void splitResult()
        {
            try
            {
                int kwStart = 0;

                for (int i = 1; i <= 8; i++)
                {
                    kwStart = resultMsg.IndexOf($"0{i}0404");
                    if (kwStart != -1 && kwStart != 0) break;
                }
                if (resultMsg.Length == 0 || kwStart == -1)
                {
                    return;
                }


                string wMsg = resultMsg.Substring(0, kwStart - 1);
                string kwMsg = resultMsg.Substring(kwStart);
                for (int i = 1; i <= 8; i++)
                {
                    int wStart = wMsg.IndexOf($"0{i}0402");
                    kwStart = kwMsg.IndexOf($"0{i}0404");

                    if (kwStart != -1 && kwMsg.Length - kwStart >= 18)
                    {
                        kwhValues[i - 1] = HextoFloat(kwMsg.Substring(kwStart, 18).Substring(6, 8), "kw", i - 1);
                        kwMsg = kwMsg.Substring(kwStart + 18);
                    }
                    if (wStart != -1 && wMsg.Length - wStart >= 14)
                    {
                        wValues[i - 1] = HextoFloat(wMsg.Substring(wStart, 14).Substring(6, 4), "w", i - 1);
                        wMsg = wMsg.Substring(wStart + 14);
                    }
                }
            }
            catch(ArgumentException ae)
            {
                Log($"sen 4 timer2_tick 에러 : {ae}");
            }
            
            UpdatePerTextBox();
            dbHelper.insertElect(kwhValues, "MKWH");
            dbHelper.insertElect(wValues, "MW");

        }
        private void Log(string message)
        {
            // 디버그 출력 (파일이나 다른 목적지에 로그 기록)
            Debug.WriteLine($"{DateTime.Now}: {message}");
        }
    }
}
