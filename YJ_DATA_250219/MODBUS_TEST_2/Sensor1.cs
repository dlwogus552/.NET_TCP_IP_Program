    ...
    public partial class Sensor1 : Form
    {
        ...
        private void StartListening()
        {
            try
            {
                ...
                // 새로운 리스너 시작
                _listener = new TcpListener(IPAddress.Any, 6066);
                _listener.Server.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, true);
                _listener.Start();
                _isListening = true;
                _isClientConnected = false;
                Log("서버가 6066 포트에서 시작되었습니다...");
            ...
            {
                Log("sen 1 StartListening 예외 발생: " + ex.Message);
            }
        }
        ...
        private async void ListenForClients(CancellationToken cancellationToken)
        {
            try
            {
                while (_isListening && !cancellationToken.IsCancellationRequested)
                {
                    ...
                    {
                        Log("sen 1 TcpListener가 중지되었습니다.");
                    }
                    catch (Exception ex)
                    {
                        Log("sen 1 ListenForClients 예외 발생: " + ex.Message);
                    }
                }
            }
            finally
        ...
        private async Task HandleClient(TcpClient client)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        Log("sen 1 클라이언트의 Keep-Alive 타임아웃 발생, 연결 끊어짐");
                        break;
                    }
                }
                Log($"sen 1 클라이언트 연결 끊어짐");
            }
            catch (Exception e)
            {
                Log("sen 1 HandleClient 예외 발생: " + e.Message);
            }
            finally
            {
                stream.Close();
                client.Close();
                Log("sen 1 클라이언트 연결 해제됨");
        ...
        private void CheckForTimeout(object state)
        {
            ...
            {
                Log("sen 1 타임아웃 발생: 포트를 닫았다가 다시 엽니다.");
                RestartListening();
            }
        }
        ...
        private void StopListening()
        {
            ...
            try
            {
                ...
                Log("sen 1 서버가 중지되었습니다...");
            }
            catch (Exception ex)
            {
                Log("sen 1 StopListening 예외 발생: " + ex.Message);
            }
        }
